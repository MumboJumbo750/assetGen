...
Use this as a stable â€œstyle lockâ€ prefix:
...
Notes:
- Text-to-image may not perfectly align frames; treat these as â€œbest effortâ€ until we add a dedicated spritesheet workflow.
...
Notes:
- Strongly emphasize â€œoverlay only / no body redrawâ€.
...
For now we treat â€œtransparent backgroundâ€ as â€œplain/empty backgroundâ€; later we can add a background-removal step to the workflow once generation quality is acceptable.

## How to use this with AssetGen
- If youâ€™re using the generator, it already generates prompts per asset path.
- This file is the â€œstyle targetâ€ prompt language for Juggernaut XL; you can:
  1) keep it as a human reference, or
  2) update the generatorâ€™s `STYLE_HEADER` / prompt templates to match this pack if you want stronger Juggernaut-optimized phrasing.
