...
export async function createGame({ pixi, root = "../../..", input, bus }) {
  ...
  const SCORE_KEY = "zelos:space-shooter:highscore";
  const ACHIEVEMENTS_KEY = "zelos:space-shooter:achievements";
  ...
  function layoutScene() {
    ...
    if (planet) {
      planet.scale.set(0.6);
      planet.position.set(width * 0.2, height * 0.25);
    }

    if (satellite) {
      satellite.scale.set(0.6);
      satellite.position.set(width * 0.82, height * 0.2);
    }
  ...
  function applyGlowSettings() {
    ...
    if (background) background.alpha = 0.75 * glowScale;
    if (planet) planet.alpha = 0.8 * glowScale;
    if (satellite) satellite.alpha = 0.9 * glowScale;
  }
  ...
  app.ticker.add(() => {
    ...
    const speed = 4 + difficulty * 2;
    player.x += state.input.axisX * speed;
    player.y += state.input.axisY * speed;
