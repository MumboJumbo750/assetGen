param(
    ...
    [string]$Checkpoint = "animagineXLV31_v31.safetensors",
    [string]$Variant = "animagine",
    [int]$MaxRenderDim = 768,
    [int]$Seed = 12345,
    [int]$ComfyStartTimeoutSec = 180,
    [switch]$SkipComfyUi,
    [switch]$Cpu,
    [switch]$DryRun
)
...
function Test-ComfyUi {
    ...
    try {
        # Use a lightweight endpoint. /system_stats is available on recent ComfyUI.
...
function Ensure-ComfyUiRunning {
    ...
    # If we have a previous pid, assume it's the intended server and wait for it to become ready.
            ...
            if ($oldPid -match '^\d+$') {
                ...
                if ($p) {
                    ...
                    while ((Get-Date) -lt $deadline) {
                        ...
                        # If the process exits while we wait, fall through to start a new one.
    ...
    # Start ComfyUI detached so subsequent commands don't interrupt it.
    $args = @(
        '-NoProfile',
...
Write-Host "[2/2] Generating Animagine sample set into: $variantRoot"
